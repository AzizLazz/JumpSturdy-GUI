...
public class Game extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        Button btn2 = new Button("Get Best Move Suggestion from AI Bot");
        Button btn4 = new Button("Red Move");
        Button btn5 = new Button("Blue Move");
        Button btn6 = new Button("Game");
        ...
        // Move Suggestion button action
        btn2.setOnAction(event -> {
            String fen = toFEN();
            String bestMove = turn ? AIBotClient.getBestMoveFromServer_2(fen, RedPort,turn) : AIBotClient.getBestMoveFromServer_2(fen, BluePort,turn);
        ...
        // Red Move button action
        btn4.setOnAction(event -> {
            String fen = toFEN();
            String bestMove = AIBotClient.getBestMoveFromServer_2(fen, RedPort,turn);
            moveIndex = getMoveIndexes(bestMove);
            squares[moveIndex[0]][moveIndex[1]].fire();
            squares[moveIndex[2]][moveIndex[3]].fire();
            System.out.println("Bot played: " + bestMove);
        });

        // Blue Move button action
        btn5.setOnAction(event -> {
            String fen = toFEN();
            String bestMove = AIBotClient.getBestMoveFromServer_2(fen, BluePort,turn);
            moveIndex = getMoveIndexes(bestMove);
            squares[moveIndex[0]][moveIndex[1]].fire();
            squares[moveIndex[2]][moveIndex[3]].fire();
            System.out.println("Bot played: " + bestMove);
        ...
        // Game loop button action
        btn6.setOnAction(event -> {
            PauseTransition pause = new PauseTransition(Duration.seconds(1));
            pause.setOnFinished(e -> {
                if (currentWinningState().equals("Ongoing")) {
                    if (turn) {
                        btn4.fire();
                    } else {
                        btn5.fire();
        ...
        buttonBox.getChildren().addAll(btn1, btn2, btn4, btn5, btn6);
        buttonBox.setAlignment(Pos.CENTER);

        // Column Labels
        Label colLabels = new Label("    A    B    C    D    E    F    G    H");
        colLabels.setFont(new Font("Arial", 58));
        colLabels.setTextFill(Color.BLACK);

        // Row Labels
        VBox rowLabels = new VBox(20);
        for (int i = 1; i <= 8; i++) {
            Label rowLabel = new Label(String.valueOf(9-i));
            rowLabel.setFont(new Font("Arial", 70));
            rowLabel.setTextFill(Color.BLACK);
            rowLabels.getChildren().add(rowLabel);
        }


        HBox boardWithLabels = new HBox(10);
        boardWithLabels.getChildren().addAll(rowLabels, board);

        VBox root = new VBox();
        root.getChildren().addAll(colLabels, boardWithLabels, buttonBox, statusLabel);

        Scene scene = new Scene(root, 870, 950);
        primaryStage.setTitle("JumpSturdy GUI by AzizLazz");
    ...
    public int[] getMoveIndexes(String move) {

        String[] positions = move.split("-");
        if (positions.length != 2) {
            throw new IllegalArgumentException("Invalid move format. Expected format: 'G5-G6'");
        }
        ...
        int fromCol = from.charAt(0) - 'A'; // Convert letter to column index (A=0, B=1, ...)
        int fromRow = 8 - Character.getNumericValue(from.charAt(1)); // Convert number to row index (1=7, 8=0)
